...
pub enum TextOrMedia {
    ...
    Text(#[garde(length(max = crate::config::answer_text::MAX_LENGTH))] String),
}

/// A fuiz configuration, title is unused
#[derive(Debug, Serialize, Deserialize, Clone, Validate)]
pub struct Fuiz {
    #[garde(length(max = crate::config::fuiz::MAX_TITLE_LENGTH))]
    title: String,

    #[garde(length(max = crate::config::fuiz::MAX_SLIDES_COUNT), dive)]
